...
import { verifyResetCodeEnseignant } from '../../api/enseignant/Auth.enseignant.api';
...
const VerifyCode = () => {
  ...
  const [message, setMessage] = useState('Code de réinitialisation envoyé à votre email.');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = await verifyResetCodeEnseignant(email, code);
      setMessage(data.message || 'Code valide.');

      setTimeout(() => {
        navigate('/enseignant/reset-password', {
          state: { email, code },
        });
      }, 900);
    } catch (err) {
    ...
    <div className="reset-auth-container">
      ...
      <div className="reset-auth-right">
        <div className="reset-auth-header">
          <h1 className="reset-auth-title">Réinitialiser mot de passe enseignant</h1>
        ...
        <form onSubmit={handleSubmit} className="reset-auth-form" noValidate>
          ...
          <div className="reset-step-badge">Étape 2/3: Vérifier code</div>

          <div className="reset-form-group">
            <label htmlFor="verify-email">Adresse email</label>
          ...
          <div className="reset-form-group">
            <label htmlFor="verify-code">Code reçu par email</label>
            <div className="reset-input-wrap">
              <span className="reset-input-icon">
                <IconCode />
              </span>
