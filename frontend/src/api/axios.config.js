  ...
  (error) => {
    ...
    if (error.response?.status === 401) {
      const isLoginAttempt = 
        ...
        error.config?.url?.includes('/enseignant/auth/login');

      // Si ce n'est pas une tentative de connexion, rediriger
      if (!isLoginAttempt) {
        let redirectPath = '/';
        try {
          const storedUser = JSON.parse(localStorage.getItem('user') || '{}');
          if (storedUser?.role === 'enseignant') {
            redirectPath = '/enseignant/login';
          }
        } catch {
          redirectPath = '/';
        }
